...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
  ...
  // cupsArrayNew()
  // No copy/free callbacks: the array stores the harness-owned string pointers directly
  // (the harness frees first_string/second_string itself). Using _cupsArrayStrdup/_cupsArrayFree
  // here triggers a leak via cupsArrayDup(), which does not propagate the free callback to the
  // duplicate, so the dup-array elements can never be freed - a fixture leak on every input.
  array = cupsArrayNew3((cups_array_func_t)_cupsArrayStrcmp, (void *)first_string, NULL, 0, NULL, NULL);
